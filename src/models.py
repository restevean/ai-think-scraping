...
class Message(BaseModel):
    ...
    author_initials: Optional[str] = Field(
        default=None, description="Author initials if available"
    )
    date: Optional[datetime] = Field(
        default=None, description="Message date/timestamp"
    )
...
class ConversationThread(BaseModel):
    ...
    messages: list[Message] = Field(
        default_factory=list, description="List of messages in thread"
    )
